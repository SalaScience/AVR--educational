// ***********************************************************************
// (c)2022 SalaScience
// LCD library for HD44780 driver
// ***********************************************************************
...
void LCD_init(void)
{
 ...
 _delay_us(100);
 // ==== 4-bit interface ===========
 LCD_send_whole_byte( (1 << 5) | (1 << 3));         // 4-bit interface, 2 lines, 5x8 dots per symbol
 _delay_us(100);
...
{
 uint8_t addr = 0;
 if (y == 1) addr = x;                              // first row  (address 0x00 = 0)
 if (y == 2) addr = x + 0x40;                       // second row (address 0x40 = 64)
 if (y == 3) addr = x + 0x54;                       // third row  (address 0x54 = 84)
 if (y == 4) addr = x + 0x14;                       // fourth row (address 0x14 = 20)
